...
# Drop keys that are manual REVISION numbers rather than section numbers.
# e.g. the podcast "Built from ... FOM 124.2 ..." blurb parsed as FOM section 124.2.
REV_NOISE={"FOM":lambda s:s.split('.')[0].isdigit() and int(s.split('.')[0])>=100}
for man,test in REV_NOISE.items():
    for sec in [k for k in index.get(man,{}) if test(k)]:
        del index[man][sec]
...
# Also regenerate the human-readable index. This file claims to be auto-generated,
# so generate it rather than letting it drift by hand.
_md=["# Portal Citation Index (AUTO-GENERATED)","",
...
for pg in sorted(page_manuals):
    _md.append(f"- **{pg}**: " + ", ".join(f"{m} {n}" for m,n in sorted(page_manuals[pg].items())))
_md+=["","## Untagged pages (manual content but NO parseable section refs - manual review required)",""]
for pg,ms in sorted(untagged.items()):
    _md.append(f"- **{pg}**: mentions {ms} - NO structured refs")
_md+=["- **fom_quiz / fom_q/*.json**: FOM question bank carries ref/src.ref per item but is not parsed here.",
...
open(_os.path.join("docs","CITATION_INDEX.md"),"w",encoding="utf-8").write("\n".join(_md))
print("wrote citation_index.json and docs/CITATION_INDEX.md")
